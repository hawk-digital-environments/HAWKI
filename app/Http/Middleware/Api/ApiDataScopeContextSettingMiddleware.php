...
readonly class ApiDataScopeContextSettingMiddleware
{
    ...
    public function __construct(
        private Server|null $jsonApiServer
    )
    {
    }
    ...
    {
        if ($this->jsonApiServer) {
            $this->populateContextByRequest($request);
        }
        return $next($request);
    }

    private function populateContextByRequest(Request $request): void
    {
        ...
        $schemaContainer = $this->jsonApiServer->schemas();
