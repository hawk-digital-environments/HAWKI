 ...
 * Use App\Orchid\Traits\AiConnectionTrait instead for model management operations.
 * 
 * Reasons for deprecation:
 * - Complex dependencies on AIProviderFactory and multiple services
 * - Provider-specific methods that are hard to maintain
 * - Replaced by simpler, more robust AiConnectionTrait
 * - No longer used by any screens or services
 * 
 * Migration path:
 * - Use AiConnectionTrait::fetchModelsDirectly() instead of getModelStatus()
 * - Use AiConnectionTrait::saveModelsToDatabase() instead of importModelsFromApiResponse()
 * - Use standard Eloquent operations for deleteModel() functionality
 * 
 * @see App\Orchid\Traits\AiConnectionTrait
 */
class ModelSettingsService
{
    /**
     * @var ApiProvidersService
     */
     ...
     * Constructor.
     */
    public function __construct(ApiProvidersService $apiProvidersService, AIProviderFactory $aiProviderFactory)
    {
        $this->providerSettingsService = $apiProvidersService;
     ...
     *
     * @deprecated Use App\Orchid\Traits\AiConnectionTrait::fetchModelsDirectly() instead
     * @throws \Exception
     */
    public function getModelStatus(string $providerName): array
    {
        $provider = ApiProvider::with('apiFormat.endpoints')->where('provider_name', $providerName)->first();
     ...
     * @throws \Exception
     */
    private function fetchModelsUsingAIProvider(ApiProvider $provider): array
    {
        try {
     ...
     * @throws \Exception
     */
    private function fetchModelsFromProviderDirect(ApiProvider $provider): array
    {
     ...
     *
     * @deprecated Use standard Eloquent operations: AiModel::find($id)->delete()
     ...
     *
     * @deprecated Use App\Orchid\Traits\AiConnectionTrait::saveModelsToDatabase() instead
        ...
        try {
            ...
            foreach ($models as $modelData) {
                try {
                    ...
                    $existingModel = AiModel::where('model_id', $modelId)
                    ...
                    if ($existingModel) {
                        ...
                        AiModel::create($modelAttributes);
                        $importedModels++;
