...
class PathBuilder
{
    ...
    private function buildRelatedOperation(
        ...
        // Resolve the inverse schema for this relation (e.g. for
        // /assistants/{id}/ai-tools, find the ai-tools schema and its
        // PascalCase class name so we can build the right $ref targets).
        [$inverseClassName, $inverseType, $isToMany] = $this->resolveInverse($meta, $relation, $resource);

        return match ($method) {
            'post' => $this->buildRelatedCreateOperation($resource, $className, $tag, $relation, $inverseClassName, $inverseType, $schemaMap),
            'patch' => $this->buildRelatedUpdateOperation($resource, $className, $tag, $relation, $inverseClassName, $inverseType, $schemaMap),
            'delete' => $this->buildRelatedDeleteOperation($resource, $className, $tag, $relation),
            'get' => $this->buildRelatedFetchOperation($resource, $className, $tag, $relation, $inverseClassName, $isToMany),
            default => null,
        };
    }

    /**
     * Resolve the inverse schema (class name, resource type, and to-many flag)
     * for a related-resource route. Returns a triple [$inverseClassName, $inverseType, $isToMany]
     * where $inverseClassName is null if the schema could not be resolved.
     *
     * @return array{0: null|string, 1: string, 2: bool}
     */
    private function resolveInverse(?array $meta, string $relation, string $resource): array
    {
        ...
        if (!$meta) {
            return [$inverseClassName, $inverseType, $isToMany];
        }
        ...
        foreach ($schema->fields() as $field) {
            ...
            try {
                ...
                // Inverse resolution is best-effort; the resource type falls
                // back to the parent type when reflection fails.
            }
    ...
    /**
     * POST /{resource}/{id}/{relation}: create a new related resource.
     */
    private function buildRelatedCreateOperation(
        string $resource,
        string $className,
        string $tag,
        string $relation,
        ...
        $requestExample = ($hasRelation && $relationMeta)
        ...
        $summary = $hasRelation
            ...
            : ucfirst($relationLabel) . " {$resource}";
        ...
        $operation = [
            ...
            'responses' => [
                '200' => $this->buildDataResponse($inverseClassName, 'Successful'),
                '401' => ['$ref' => '#/components/responses/Unauthorized'],
                '403' => ['$ref' => '#/components/responses/Forbidden'],
                '404' => ['$ref' => '#/components/responses/NotFound'],
            ],
    ...
    /**
     * PATCH /{resource}/{id}/{relation}: update a nested sub-resource
     * (e.g. PATCH /assistants/{id}/assistant-review).
     */
    private function buildRelatedUpdateOperation(
        string $resource,
        string $className,
        string $tag,
        string $relation,
        ...
        return [
            ...
            'responses' => [
                '200' => $this->buildDataResponse($inverseClassName, 'Updated'),
                '401' => ['$ref' => '#/components/responses/Unauthorized'],
                '403' => ['$ref' => '#/components/responses/Forbidden'],
                '404' => ['$ref' => '#/components/responses/NotFound'],
                '422' => ['$ref' => '#/components/responses/UnprocessableEntity'],
    ...
    /**
     * DELETE /{resource}/{id}/{relation}: delete the related resource.
     */
    private function buildRelatedDeleteOperation(
        string $resource,
        string $className,
        string $tag,
        string $relation,
    ...
    /**
     * GET /{resource}/{id}/{relation}: fetch the related resource(s).
     */
    private function buildRelatedFetchOperation(
        string $resource,
        string $className,
        string $tag,
        string $relation,
    ...
    /**
     * Build a 2xx response body that wraps an inverse-resource $ref under `data`.
     */
    private function buildDataResponse(?string $inverseClassName, string $description): array
    {
        $properties = $inverseClassName
            ? ['data' => ['$ref' => "#/components/schemas/{$inverseClassName}Resource"]]
     ...
     * Special-cases SSE streaming responses.
     *
     * @param string $method   HTTP method (only POST supported)
     * @param string $resource JSON:API resource type
     * @param string $tag      operation tag
     * @param string $action   The action name (e.g., "favorite").
     * @param mixed  $route    the Laravel route instance
     */
    private function buildActionOperation(
        ...
        // Actions registered with HTTP-method dispatch on the same URL
        // (e.g. POST + DELETE /assistants/{id}/actions/attachment).
        if ('delete' === $method) {
            return [
                'summary' => ucfirst(str_replace('-', ' ', $action)) . " {$resource}",
                'operationId' => "{$this->camelize($action)}{$className}",
                'tags' => [$tag],
                'parameters' => [$pathParam],
                'requestBody' => [
                    'required' => true,
                    'content' => [
                        'application/json' => [
                            ...
                            'example' => ['fileId' => 'abc123-e29b-41d4-a716-446655440000'],
                        ],
                    ],
                ],
                'responses' => [
                    '200' => [
                        'description' => 'Attachment deleted, assistant updated',
                        'content' => [
                            'application/vnd.api+json' => $this->withExample(
                                "#/components/schemas/{$className}SingleResponse",
        ...
        } elseif ('attachment' === $action && 'assistants' === $resource) {
            // File upload action with no dedicated FormRequest: the controller
            // validates the uploaded file inline, so the multipart body
            // schema is defined here manually.
            $operation['requestBody'] = [
                'required' => true,
                'content' => [
                    'multipart/form-data' => [
                        'schema' => [
                            ...
                            'properties' => [
                                'file' => [
                                    'type' => 'string',
                                    'format' => 'binary',
                                    'description' => 'File to upload as knowledge for this assistant. Supported formats are listed in the storage_files client config (images, plain text files, and converter-supported document formats such as PDF and Word).',
                                ],
                            ],
                            'required' => ['file'],
                        ],
     ...
     *
     * Reused by streaming endpoints. References
     ...
     *
     * @param string $input The input string (e.g. "ai_model").
     *
     * @return string The camelCased string (e.g. "aiModel").
     */
