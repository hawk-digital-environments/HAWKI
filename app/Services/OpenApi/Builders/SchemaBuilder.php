...
class SchemaBuilder
{
     ...
     * @param Schema $schema the JSON:API schema instance
     *
     * @return array{constraints: array<string, array>, required: list<string>, validated: list<string>} parsed constraints, required field names, and all validated field names
     */
    public function parseValidationConstraints(Schema $schema): array
    {
        ...
        try {
            // bindMockRoute returns true when it actually bound the mock (i.e.
            // no prior binding existed). We track this so we can remove it
            ...
            if ($boundLocally) {
                app()->forgetInstance(RouteContract::class);
            }
        }
     ...
     * @param Schema $schema the JSON:API schema instance
     *
     * @return null|string the Request class FQCN, or null if not found
     */
    private function resolveRequestClass(Schema $schema): ?string
    {
        ...
        $requestName = mb_substr($baseName, 0, -6) . 'Request';
    ...
    private function extractConstraints(array $rules): array
    {
        ...
        foreach ($rules as $rule) {
            if ('integer' === $rule) {
                ...
                if (str_starts_with($rule, 'min:')) {
                    $value = $isInteger ? (int) mb_substr($rule, 4) : (float) mb_substr($rule, 4);
                    $constraints['minimum'] = $value;
                } elseif (str_starts_with($rule, 'max:')) {
                    if ($isNumeric || $isInteger) {
                        $constraints['maximum'] = $isInteger ? (int) mb_substr($rule, 4) : (float) mb_substr($rule, 4);
                    } else {
                        $constraints['maxLength'] = (int) mb_substr($rule, 4);
                    }
                }
        ...
        return array_filter($constraints, static fn ($v) => null !== $v);
    }
     ...
     *
     * @return float|int a representative example value within the constraint range
     */
    private function deriveExample(array $constraints, bool $isInteger): float|int
    {
     ...
     *
     * @return null|string the enum class FQCN, or null if reflection fails
     */
     ...
     *
     * Returns true when the mock was actually registered (i.e. no prior binding
     * existed), false when a binding was already present and was left untouched.
     * Callers that receive true should remove the mock afterwards to avoid
     * leaking it into other consumers of RouteContract during the same request.
     *
     * @param Schema $schema the JSON:API schema instance to return from the mock route
     */
    private function bindMockRoute(Schema $schema): bool
    {
    ...
    private function inferType(array $rules): array
    {
        ...
        foreach ($rules as $rule) {
            ...
            if ('integer' === $ruleStr) {
                $type = 'integer';
            } elseif ('numeric' === $ruleStr) {
                $type = 'number';
            } elseif ('boolean' === $ruleStr) {
                $type = 'boolean';
            } elseif ('array' === $ruleStr) {
                $type = 'array';
            } elseif (str_starts_with($ruleStr, 'in:')) {
                $result['enum'] = explode(',', mb_substr($ruleStr, 3));
            }
        }
