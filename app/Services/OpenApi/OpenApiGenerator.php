...
class OpenApiGenerator
{
    ...
    public function generate(bool $withExamples = true): array
    {
        ...
        $componentSchemas['IncludedResource'] = [
            'oneOf' => array_map(static fn ($name) => ['$ref' => "#/components/schemas/{$name}"], $resourceNames),
        ];
     ...
     *
     * @return list<string> list of schema class FQCNs
     */
     ...
     * ErrorObject, and SSE event schemas for streaming endpoints
    ...
    {
        $errorSchema = static function (string $status, string $title, string $detail, bool $withSource = false): array {
            $example = [
                'status' => $status,
                'title' => $title,
                'detail' => $detail,
    ...
    private function buildActionRequestSchemas(): array
    {
        ...
        foreach ($routes as $route) {
            ...
            // Strip the "api/" segment and the JSON:API server prefix (e.g. "hawki/v1")
            // to normalize the path before classifying it.
            ...
            if (preg_match('#^([^/]+)/\{[^}]+\}/actions/([^/]+)$#', $path, $m)) {
                ...
                // Custom two- or three-segment endpoint, e.g. "chat/responses"
                // or "openai/v1/responses".
                $resource = $m[1];
                $action = $m[2];
            }
