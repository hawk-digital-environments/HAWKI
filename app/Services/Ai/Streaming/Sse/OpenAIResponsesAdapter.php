...
class OpenAIResponsesAdapter extends SSEAdapter
{
    ...
    public function end(): iterable
    {
        ...
        // OpenAI Responses clients (e.g. Vercel AI SDK) use the literal
        // [DONE] marker to detect end-of-stream; without it the connection
        // can appear to hang.
        yield "data: [DONE]\n\n";
    }
    ...
    {
        // The wire-facing message is generic so credentials, internal paths,
        // or vendor-specific identifiers in $e->getMessage() cannot leak to
        // the client. The full exception is logged by the controller.
    ...
    /**
     * Stable event id used in the SSE `id:` frame so clients can resume /
     * deduplicate events. Format mirrors OpenAI's `evt_<uuid>` convention.
     */
