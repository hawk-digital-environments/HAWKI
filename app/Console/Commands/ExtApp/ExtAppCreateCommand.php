...
use App\Services\ExtApp\ExtAppCreator;
use App\Services\ExtApp\Repositories\ExtAppRepository;

class ExtAppCreateCommand extends AbstractExtAppCommand
{
    public function __construct(
        private readonly ExtAppCreator    $appCreator,
        private readonly ExtAppRepository $appDb,
        ExtAppConfig                      $extAppConfig
    )
    {
        parent::__construct($extAppConfig);
    }
    ...
    public function handle(): void
    {
        ...
        $name = $this->output->ask('Please enter the name of the app (e.g. "HAWKI App")', null, function ($name) {
            ...
            if ($this->appDb->findOneByName($name)->isNotEmpty()) {
