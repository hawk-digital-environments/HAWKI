    ...
    public function columns(): array
    {
        return [
            ...
            TD::make('ai_model', 'AI Model')
                ->sort()
                ->render(function (AiAssistant $assistant) {
                    // Lade das Model falls es nicht eager loaded wurde
                    // Note: ai_model column references system_id in ai_models table
                    $model = $assistant->aiModel ?? ($assistant->ai_model ? \App\Models\AiModel::with('provider')->where('system_id', $assistant->ai_model)->first() : null);
                    ...
                    if ($model) {
                        ...
                        $provider = $model->provider ?? ($model->provider_id ? \App\Models\ApiProvider::find($model->provider_id) : null);
                        ...
                        return "<span class=\"badge {$badgeClass} border-0 rounded-pill\" title=\"{$tooltipText}\">{$model->label}</span>";
                    }
                ...
                ->render(function (AiAssistant $assistant) {
                    ...
                    if ($isSystemAssistant) {
                        ...
                        if (! $assistant->ai_model) {
                            ...
                            // Lade das Model falls es nicht eager loaded wurde
                            // Note: ai_model column references system_id in ai_models table
                            $model = $assistant->aiModel ?? \App\Models\AiModel::where('system_id', $assistant->ai_model)->first();
                            ...
                            if ($model) {
                                ...
                                $provider = $model->provider ?? \App\Models\ApiProvider::find($model->provider_id);
                                if ($provider && ! $provider->is_active) {
                                    $warnings[] = 'Provider Inactive';
                                }
                            } else {
                                // Model wurde gelöscht oder existiert nicht mehr
                                $warnings[] = 'AI Model Not Found';
                            }
                        }
