...
function formatMessage(rawContent, groundingMetadata = '', isGenerating = false) {
  ...
  try {
    ...
    finalContent = convertHyperlinksToLinks(finalContent, isGenerating);
...
//#region Links Formatter
// Efficiently convert URLs to links while respecting excluded areas
function convertHyperlinksToLinks(text, isGenerating = false) {
    ...
    function processTextNodes(node) {
        ...
        if (child.nodeType === Node.TEXT_NODE && child.nodeValue && child.nodeValue.match(URL_REGEX)) {
            ...
            while ((match = regex.exec(child.nodeValue)) !== null) {
                ...
                const link = createUrlBlock(uid, url);
                if(!isGenerating){
                    renderUrl(uid, url);
                }
...
function createUrlBlock(uid, url){

    // Create the Element
    const temp = document.getElementById('inline-link-content-template')
    const clone = temp.content.cloneNode(true);
    const link = clone.querySelector(".inline-link");

    if (!link) {
        throw new Error('Template must contain an element with class "inline-link".');
    }
    link.dataset.uid = uid;
    link.querySelector('.adr-url').href = url;
    link.querySelector('.adr-url').innerText = url;

    return link;
}

function renderElementUrls(messageElement){
    const links = messageElement.querySelectorAll('.inline-link');
    links.forEach(link => {
        renderUrl(link.dataset.uid,
                  link.querySelector('.adr-url').href);
    });
}

async function renderUrl(uid, url){
    console.log(uid);

    //check for metadata
    let metadata;
    if(metaList[uid]){
        metadata = metaList[uid];
    }
    else{
        ...
        metaList[uid] = metadata;
    }
    const element = document.querySelector(`.inline-link[data-uid="${uid}"`);
    console.log(element);
    const titleEl = element.querySelector('.title');
    titleEl.innerText = metadata.title ? metadata.title : "";

    const iconEl = element.querySelector('.adr-icon');

    if(metadata.favicon != null){
        iconEl.setAttribute('src', metadata.favicon);
    }

    if(metadata.image != null){
        element.querySelector('.link-cover').setAttribute('src', metadata.image)
    }
...
async function fetchUrlMetadata(url) {
  try {
    const response = await fetch(`/req/fetchOgMeta?url=${encodeURIComponent(url)}`);
