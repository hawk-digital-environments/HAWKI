...
If you are using the Docker deployment, please compare your current setup with the new `_docker_production` directory,
as there are some changes needed to update to version 2.1.0:

* The `.env` file has some new variables that need to be added.
* The `model_providers.php` file has a changed format and was moved to another location.
* The `model_lists` directory has been added.
* New services were introduced to the `docker-compose.yml` file (the scheduler and file-converter).
