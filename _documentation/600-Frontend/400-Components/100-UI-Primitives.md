...
| `RouterView`, `RouteError`, `RouteNotFound` + routing kit | `ui/routing/` | The SPA routing kit: `RouterView` renders the matched route; `RouteError`/`RouteNotFound` handle failures; `hooks/` (`useRouter`, `useRouteMeta`), `logistics/` (`RouteRegistrar`, `router.svelte`, `buildMiddlewareStack`, `lazyComponent`, `isActive`, `layouts`), and `strategy/` (`path`, `hash`, `transient`) round out the kit. See [Architecture → Modules & Routing](../300-Architecture/120-Modules-and-Routing.md). |
