...
Why not the AI streaming path? `StreamController.handleGroupChatRequest` is architecturally more interesting but currently mixes domain logic, encryption, and broadcasting in a 130-line method with inline validation. The [Technical Debt Register](../900-Technical-Debt.md) documents this. Showing it as the tutorial example would teach the wrong pattern. The AI path is covered end-to-end in [AI Service Layer](../400-Domains/100-AI/index.md).
:::
...
| AI path (agent dispatch) | `AgentRegistry` → `ChatAgentFromLegacyRequestFactory` → `ChatAgent::sendStreaming()` — see [AI Service Layer](../400-Domains/100-AI/index.md) |
| JSON:API resources | `app/JsonApi/V1/` schemas + `app/Http/Resources/` serializers — see [JSON:API Conventions](../300-HTTP-API/100-JSON-API-Conventions.md) |
