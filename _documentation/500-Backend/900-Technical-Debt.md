...
## Architecture debt — pending admin panel

Items that will be resolved when the new admin panel lands. Not coding-standard violations, but patterns scheduled for replacement.

| Location | What | Replacement | Status |
|---|---|---|---|
| `app/Services/Ai/Tools/FunctionToolSyncer.php` | Syncs function tools from config files to DB; deprecated | Registry-based discovery + UI configuration via admin panel | Scheduled for removal |
| Tool registration via container tag (`$app->tag([MyTool::class], 'ai.tool')`) | Not a registry pattern like `ProviderAdapterRegistry` / `AgentRegistry` | Will become a proper tool registry | Subject to change |
| `ai:config:sync` and static config files (`config/model_providers.php`, `config/model_lists/`) | Config-file-based provider/model sync; legacy | UI-based configuration via admin panel | Scheduled for removal |
