...
class AssistantPolicyTest extends TestCase
{
     ...
     * (creator or org admin); shared users and other viewers are excluded.
     */
    ...
    #[DataProvider('privilegedIncludes')]
    public function testIndexSensitiveIncludeNarrowsToPrivilegedAssistants(string $include, string $type): void
    {
        ...
        self::assertNotEmpty(
            collect($response->json('included') ?? [])->where('type', $type),
            "include={$include} did not surface a \"{$type}\" resource for the privileged owner",
        );

        // A public viewer is privileged for none, so they see an empty set.
        $public = User::factory()->create();
        $this->actingAsUser($public);
        $this->jsonApiRaw('get', "/api/hawki/v1/assistants?include={$include}")
            ->assertOk()
    ...
    private function seedChildren(Assistant $assistant): void
    {
        ...
        AssistantFeedback::forceCreate(['assistant_id' => $assistant->id, 'user_id' => $assistant->creator_id, 'text' => 'nice']);
