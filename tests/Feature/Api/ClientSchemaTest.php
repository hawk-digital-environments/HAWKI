...
class ClientSchemaTest extends TestCase
{
    ...
    public function testWriteOnlyResourcesDoNotAdvertiseRelationshipFetch(): void
    {
        $this->actingAsUser($this->user);

        $response = $this->jsonApiRaw('get', '/api/hawki/v1/assistants/schema');

        // assistant-feedback exposes the assistant + user relationships only for
        // writes; neither has a fetch endpoint advertised to the client.
        $feedbackRels = $response->json('resources.assistant-feedback.relationships');
        self::assertArrayNotHasKey('endpoints', $feedbackRels['assistant']);
        self::assertArrayNotHasKey('endpoints', $feedbackRels['user']);

        // assistant-user-prompts likewise exposes assistant only for writes.
        $promptRels = $response->json('resources.assistant-user-prompts.relationships');
        self::assertArrayNotHasKey('endpoints', $promptRels['assistant']);
    }
    ...
    public function testActionsExist(): void
    {
        ...
        $expectedActions = ['remix', 'favorite', 'attachment'];
    ...
    public function testActionUrls(): void
    {
        ...
        self::assertSame('/api/hawki/v1/assistants/{id}/actions/attachment', $actions['attachment']['url']);
    }
    ...
    public function testActionInputSchemas(): void
    {
        ...
        // attachment has no FormRequest → no input schema
        self::assertArrayNotHasKey('input', $actions['attachment']);
    }
