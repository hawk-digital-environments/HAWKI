...
import type {PluginAspect} from '$lib/kernel/plugins/PluginAspect.js';
...
// @todo if configuration aspect changes, it could become part of HawkiPluginContext (as it is basically a part of the client).

export interface HawkiPluginContext {
    client: HawkiClient;
    bootstrapper: Bootstrapper;
    plugins: PluginAspect;
}

export interface HawkiPluginContextWithConfig extends HawkiPluginContext {
    config: HawkiAppAspects['config'];
}

export type AppAspectRegistrar = Pick<HawkiApp, 'addAspect'>;
...
export interface HawkiPlugin {
    ...
    aspects?(registrar: AppAspectRegistrar, context: HawkiPluginContext): void | Promise<void>;
    ...
    // Executed after all plugins have been initialized and all app aspects are loaded.
