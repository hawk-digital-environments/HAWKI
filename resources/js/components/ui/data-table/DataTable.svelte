<script lang="ts" generics="T extends {id: string}">
    ...
    let {data, columns, caption, loading = false, server = false, total, pagination = $bindable({pageIndex: 0, pageSize: 25}), sorting = $bindable([]), filters = [], columnFilters = $bindable([]), selection = $bindable({}), selectable = false, rowLabel = row => row.id, onChange, actions, cells, empty}: Props = $props();
    ...
    <div class="table-scroll" tabindex="0" role="region" aria-label={caption}>
        <table>
            ...
            <tbody>
                ...
                {:else}
                    <tr><td class="empty" colspan={columns.length + Number(selectable) + Number(!!actions)}>{loading ? __('ui.loading') : empty ?? __('admin.table.empty')}</td></tr>
                {/each}
            </tbody>
        </table>
    </div>
