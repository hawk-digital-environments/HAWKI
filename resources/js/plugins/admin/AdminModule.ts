...
export class AdminModule implements HawkiModule {
    readonly name = 'admin';
    title(translate: Translator['translate']) { return translate('admin.title'); }
    icon() { return Settings01Icon; }
    sidebar() { return AdminSidebar; }
    visible(app: HawkiApp) { return app.can('admin.access'); }

    routes(registrar: RouteRegistrar) { registerAdminRoutes(registrar); }
}
