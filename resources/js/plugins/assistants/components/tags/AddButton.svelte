<script lang="ts">
    ...
    let {
        ...
        disabled?: boolean;
        /**
         * Label for the collapsed "add" button, for contexts where the items
         * are not tags ("Add prompt"). Falls back to the tag translations,
         * which also remain the fallback for the input/confirm aria-labels.
         */
        addItemLabel?: string;
        onAdd?: (value: string) => void;
    }>();

    const inputAriaLabel = $derived(addItemLabel ?? __('assistants.builder.general.tag_input_aria'));
    const confirmAriaLabel = $derived(addItemLabel ?? __('assistants.builder.general.tag_confirm_aria'));
    const collapsedLabel = $derived(addItemLabel ?? __('assistants.builder.general.tag_add'));
...
<div class="add-btn-wrapper">
    ...
    {:else}
         <button class="add-button" onclick={() => switchMode(true)} disabled={disabled} >
             ...
             <span class="label">{collapsedLabel}</span>
         </button>
    {/if}
</div>
