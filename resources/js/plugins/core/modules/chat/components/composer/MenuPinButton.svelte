<!--
  @component The pin toggle shown on a row in `AssistantMenu` / `ToolMenu` / `ModelPicker`.

  A small icon button that pins or unpins the row's assistant/tool/model via the
  ...
  it reads `--dropdown-item-action-opacity` from the menu row it sits in, so the row decides
  when its secondary controls surface; a pinned row keeps it visible so the pin state is
  always readable. The `detail` variant is the same control for the
  ...
  ## Usage
  Rendered inside a row's `DropdownMenuCheckboxItem` (or the model picker's
  `DropdownMenuRadioItem`), next to the info button:
  ```svelte
...
<script lang="ts">
    ...
    interface Props {
        ...
        /** The row's stable id: the assistant's id, the tool's name, or the model's `model_id`. */
        id: string;
...
<style>
    .menu-pin-button {
        ...
        /* Unpinned rows keep the affordance out of the way until the row is hovered or
           focused, which the menu row reports through this property. */
        opacity: var(--dropdown-item-action-opacity, 0);
    }
